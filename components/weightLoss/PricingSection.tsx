...
function saveGuestCartItem(plan: PlanRecord, variant?: PlanVariant) {
  ...
  localStorage.setItem("guest_cart_item", JSON.stringify({
    ...
    price: variant ? getActivePrice(variant) : plan.cartPrice,
    variantId: variant?.id ?? null,
    variantLabel: variant?.duration_label ?? null,
  }));
}
        ...
        {entryPlan && (
          <div className="mb-8 bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-6 relative overflow-hidden">
            ...
            <div className="text-center sm:text-left pl-0 sm:pl-2">
              <span className="inline-block bg-blue-50 text-blue-700 text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest mb-3">
                Start Here
              </span>
            ...
            if (isFeatured) {
              return (
                <div
                  ...
                  style={{
                    // Darkened + narrowed the light-lime stop so white text stays
                    // readable across the whole top of the card (the original
                    // #a8d959 start was under 2:1 contrast with white text).
                    background:
                ...
                >
                  ...
                  {/* subtle dark scrim behind the header so title/description
                      stay legible regardless of where they land on the gradient */}
            ...
            return (
              ...
              >
                ...
                <div className="mb-2 pt-3">
                  ...
                  <div className="mt-6 flex items-baseline gap-1">
                    <span className="text-4xl font-extrabold text-navy-900 tracking-tight">
                      {plan.price}
                    </span>
                    {plan.cadence && (
        ...
        <div className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-slate-200 shadow-[0_-4px_20px_rgba(0,0,0,0.1)] px-4 py-3 sm:py-4">
          <div className="max-w-6xl mx-auto flex items-center justify-between gap-4">
            <div className="min-w-0 flex-1">
              ...
              <p className="text-lg font-extrabold text-slate-900">₹{guestSelectedPlan.price.toLocaleString("en-IN")}</p>
            </div>
            <button
              onClick={async () => {
                ...
                selectedPlanForModal.variants.map((variant) => {
                  ...
                  return (
                    <button
                      ...
                      className="flex items-center justify-between p-4 rounded-2xl border-2 border-slate-200 hover:border-teal-500 hover:bg-teal-50/20 text-left transition duration-200 cursor-pointer relative overflow-hidden"
                    >
                      {hasDiscount && variant.discount_text && (
                        <div className="absolute top-0 right-0 bg-rose-500 text-white text-[9px] font-extrabold px-3 py-0.5 rounded-bl-xl uppercase tracking-wider flex items-center gap-1">
